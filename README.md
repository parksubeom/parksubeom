...
성능·안정성 문제를 만날 때마다 **어떤 선택지에서 무엇을·왜 골랐고 결과가 어땠는지**를 기록했습니다. 수치는 모두 계측으로 검증했습니다.

**① 슬라이드 이동 성능 — 2.8초 → 0.3초 (약 9배)**
- 상황: 슬라이드를 넘길 때마다 현재 화면을 통째로 이미지로 저장하느라 전환이 ~2.8초 멈췄다.
- 어떻게: 화면 전체를 한 번에 저장하던 방식을 버리고 화면 구성요소(수식·필기·3D)를 따로 합성하도록 바꾸고, "실제로 편집했을 때만" 저장하게 했다.
- 왜: 병목이 화질(해상도)이 아니라 "화면을 바꾸기 직전에 저장이 끝나길 기다리는" 구조였기 때문. 저장을 화면 전환과 분리하면 대기 자체가 사라진다.
- 효과: 전환 ~2.8초 → ~0.3초, 보기만 할 땐 기존 썸네일 유지.

**② WebGL 렌더 부하 — 13 → 51fps (약 4배)**
- 상황: 3D 교구가 화면이 그대로여도 매 순간 다시 그려져, 여러 개가 겹치면 끊겼다.
- 어떻게: "상태가 바뀔 때만 다시 그리기"로 전환하고, 카메라를 움직이는 동안만 잠깐 연속으로 그린 뒤 멈추게 했다.
- 왜: 낭비의 원인은 아무것도 안 바뀐 순간에도 계속 그리는 것. 가만히 있을 때 0프레임이면 성능 저하 없이 필요할 때만 그린다.
- 효과: 13 → 51fps, 유휴 상태 오버헤드 0.

**③ 발표 중 학생 화면이 하얗게 꺼짐 — 임시책 대신 근본 해결**
- 상황: 브라우저가 동시에 띄우는 3D 화면 수에 한계(16개)가 있는데, 발표자·학생 두 창이 나눠 쓰다 교구 9종×2창=18개로 한도를 넘겨 학생 화면이 꺼졌다.
- 어떻게: 응답 대기시간을 늘려 버티는 임시책 대신, 여러 교구가 3D 화면 하나를 공유하도록 근본을 고쳐 사용량을 창당 9개 → 1개로 줄였다(문제 시 즉시 되돌릴 스위치도 마련).
- 왜: 대기시간을 늘려도 16개 한계는 그대로라 결국 재발한다. 한도 자체를 넘지 않게 해야 근본 해결이다.
- 효과: 3D 화면 18개 → 2개, 백지·강제종료 해소.

**④ 3D 교구 소멸 버그 — 일괄 수정 대신 정밀 수정**
- 상황: 한 버그를 잡으려 공통 로직에 일괄 수정(중복 제거)을 넣으려 했다.
- 어떻게: 일괄 수정을 폐기하고, 문제가 확인된 특정 경로만 정밀하게 고쳤다.
- 왜: 배포 전 여러 각도로 검증하니 그 일괄 수정이 드래그·회전·되돌리기 등 190여 곳에서 교구가 전부 사라지게 만들었다. 한 증상을 잡으려다 더 큰 붕괴를 부르면 비용 > 이득.
- 효과: 광범위한 붕괴를 배포 전에 차단하고 원래 증상만 안전하게 해결.

**⑤ 렌더 실패 — 위험한 근본 수정 대신 안전장치 먼저**
- 상황: 특정 상황에서 3D 교구가 아무 오류 없이 화면에서 사라졌다(그릴 대상이 비어 조용히 실패).
- 어떻게: 원인(중복 초기화)을 통째로 고치는 대신 "비어 버린 화면을 그 자리에서 되살리는" 작은 가드를 먼저 넣어 증상을 즉시 없애고, 근본 수정은 별도 작업으로 분리했다.
- 왜: 근본 수정은 초기화·되돌리기·슬라이드 전환 전반을 건드려 검증 범위가 넓고 위험하다. 가드는 화면 복구 지점만 건드려 위험이 국소적이다.
- 효과: 증상 즉시 해소 + 전체 테스트 무회귀 통과, 근본은 위험을 분리해 관리.

**⑥ 판단 근거 — 눈대중 대신 계측**
- 상황: 원격 화면은 전송 지연 탓에 성능이 실제와 다르게 느껴졌다.
- 어떻게: 처리 시간·프레임 주기·3D 화면 사용량을 코드로 직접 측정하고 켜고 끄며 A/B로 비교했다.
- 왜: 체감이 아니라 숫자로 봐야 원인·효과가 확정된다. 실제로 처음 세운 원인 가설(누적 문제)이 이 계측으로 틀렸음이 드러나 진짜 원인(두 창이 GPU를 나눠 씀)을 찾았다.
- 효과: 특정 조작 시 화면 멈춤 16회 → 1회를 수치로 확인한 뒤 배포.

**일하는 방식**
- 선택지와 위험도를 표로 정리해 기획자·PM에게 쉬운 말로 먼저 설명 → 논의를 빠르게 매듭지음.
- 문제를 고칠 때마다 방어 코드와 재발 방지 테스트를 함께 남기고(정적 검사·런타임·통합 3층), 유닛·통합 테스트 800개 이상으로 회귀를 상시 차단.
- 내 가설이 측정으로 틀리면, 그 가설로 만든 수정과 테스트를 함께 되돌리고 원인을 다시 조사했다(지적 정직성).

**의의**

문제를 스스로 정의하고(계측), 근거로 설득하고(선택지 비교표), 재발까지 막는(테스트) 사이클을 상용 서비스에서 반복하며, 성능·구조 문제를 판단 근거와 함께 해결한 기록입니다.
