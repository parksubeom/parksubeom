...
성능·안정성 문제를 만날 때마다 **"쉬운 임시방편 vs 근본 해결" 중 무엇을 왜 골랐는지**를 기록했고, 결과는 모두 수치로 검증했습니다.

**① 슬라이드를 넘길 때마다 2.8초 멈추던 문제 → 0.3초 (약 9배)**
슬라이드를 넘길 때마다 현재 화면을 통째로 이미지로 저장하느라 전환이 ~2.8초 멈췄습니다. 원인은 화질이 아니라 "화면을 바꾸기 직전에 저장이 끝나길 기다리는" 구조였습니다. 저장을 화면 전환과 분리하고, 실제로 편집했을 때만 저장하도록 바꿔 전환을 ~0.3초로 줄였습니다.

**② 3D 교구가 많으면 끊기던 화면 → 13 → 51fps (약 4배)**
3D 교구는 화면이 그대로여도 매 순간 다시 그려져, 여러 개가 겹치면 뚝뚝 끊겼습니다. "바뀔 때만 다시 그리기"로 전환해 가만히 있을 땐 아무 작업도 하지 않게 했고, 성능 저하 없이 4배 부드러워졌습니다.

**③ 발표 중 학생 화면이 하얗게 꺼지던 문제 → 근본 해결**
브라우저가 동시에 띄울 수 있는 3D 화면 수에는 한계가 있는데, 발표자·학생 두 창이 이를 나눠 쓰다 한도를 넘기면 학생 화면이 꺼졌습니다. 잠깐 버티게 하는 임시책 대신, 여러 교구가 3D 화면 하나를 공유하도록 근본을 고쳐 사용량을 창당 9개 → 1개로 줄였습니다. (문제 시 즉시 되돌릴 스위치도 함께 마련)

**④ "한 번에 고치기"가 부를 대형 오류를 배포 전 차단**
한 버그를 잡으려 공통 로직에 일괄 수정을 넣으려 했지만, 배포 전 여러 각도로 검증해 보니 그 수정이 드래그·회전·되돌리기 등 190여 곳에서 교구가 전부 사라지게 만들었습니다. 일괄 수정을 폐기하고 문제 경로만 정밀하게 고쳐, 넓은 범위의 붕괴를 막고 원래 증상만 안전하게 해결했습니다.

**⑤ 원인은 크지만 위험한 수정 → 안전장치 먼저, 근본은 분리**
특정 상황에서 3D 교구가 아무 오류 없이 화면에서 사라졌습니다. 원인을 통째로 고치면 손댈 범위가 너무 넓어, 우선 "비어 버린 화면을 그 자리에서 되살리는" 작은 안전장치로 증상을 즉시 없애고 전체 테스트를 통과시킨 뒤, 근본 수정은 위험을 분리해 별도로 진행했습니다.

**⑥ 눈대중 대신 계측으로 판단**
원격 화면은 전송 지연 탓에 성능이 실제와 다르게 느껴졌습니다. 그래서 처리 시간·프레임 주기·3D 화면 사용량을 코드로 직접 측정하고 켜고 끄며 A/B로 비교했습니다. 덕분에 처음 세운 원인 가설이 틀렸음을 숫자로 확인해 진짜 원인을 찾았고, 특정 조작 시 화면 멈춤을 16회 → 1회로 줄인 것을 수치로 검증한 뒤 배포했습니다.
...
- 내 가설이 측정으로 틀리면, 그 가설로 만든 수정과 테스트를 함께 되돌리고 원인을 다시 조사했습니다(지적 정직성).
